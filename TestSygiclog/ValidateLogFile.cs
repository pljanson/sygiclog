//-----------------------------------------------------------------------
// <copyright file="ValidateLogFile.cs" company="PLJ">
...
using Sygiclog;
                ...
                if (this.passed)
                {
                    ...
                    while (readline != null)
                    {
                        ...
                        else if (readline.StartsWith("wrote trkpt : "))
                        {
                            ...
                            this.trkpts = Convert.ToInt16(strkpt);
                        }
                        else if (readline.StartsWith("Corrected Start Time = ")) // Corrected Start Time = 17-7-2012 13:57:55
                        {
                            this.corStartTimeString = readline.Substring(23); // "17-7-2012 13:57:55"
                        }
                        else if (readline.StartsWith("startLogDescription p[")) // startLogDescription p[27|1B]    [E20  Malmö]
                        {
                            ...
                            this.startLogDescription = scutoff.Substring(idx); // "[E20  Malmö]"
                        }
                        ...
                        else if (readline.StartsWith("endLogDescription p[")) // endLogDescription p[49|31]    [E20  Malmö]
                        {
                            ...
                            int idx = scutoff.IndexOf('[');
                            this.endLogDescription = scutoff.Substring(idx); // "[E20  Malmö]"
                        }
    ...
    public class Test1
    {
        ...
        /// </summary>
        [Test]
        public void SygicTravelbookLogCs()
        {
            Assert.AreEqual("Sygiclog v1.5.0beta", SygicTravelbookLog.SygiclogVersionString, " SygicTravelbookLog version sTitle");
