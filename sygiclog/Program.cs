...
{
	public const string sTitle = "Sygiclog v1.5.0";
	...
	private static void Main(string[] args)
	{
		...
		else
		{
			...
			else
			{
				...
				Console.WriteLine(sTitle + ", log 2 gpx");
				...
				for(int nArgs = 0; nArgs < args.Length; nArgs++)
				{
					...
					if(args[nArgs].StartsWith("tzc"))
					{
						Console.Write("[tzcSHH:MM]");
						settings.Tzc = args[nArgs];
						...
						try
						{
							string sTime = settings.Tzc.Remove(0, 3);
							...
							{
								int nSign = 1;
								int nMinutes = Convert.ToInt32(split[1]);
								...
								settings.TzcMinutes = nMinutes * nSign;
							}
						}
				...
				if(settings.All)
				{
					...
					foreach(string str in Directory.EnumerateFiles(Directory.GetCurrentDirectory()))
					{
						...
						bool bSucces = sygicLogFile.Parse();
		...
		if(settings.WaitConsole)
		{
			if(settings.All)
			{
				Console.WriteLine("number of logs parsed = " + nFilesParsed +
								  ",\t" + nFilesGood + " suceeded, \t"
								  + nFilesError + " Failed."
								 );
			}
